...
export class ContentfulDeletedMapperService {
    ...
    map() {
        ...
        const mappedIndustries = industries.deleted?.map((industry: Industry) => this.industryMapper.map(industry));

        const segments = readJsonFile('sync/exports/segment', 'data.diff.json');
        const mappedSegments = segments.deleted?.map((segment: Segment) => this.segmentMapper.map(segment));

        const producers = readJsonFile('sync/exports/producer', 'data.diff.json');
        const mappedProducers = producers.deleted?.map((producer: Producer) => this.producerMapper.map(producer));

        const countries = readJsonFile('sync/exports/country', 'data.diff.json');
        const mappedCountries = countries.deleted?.map((country: Country) => this.countryMapper.map(country));

        const features = readJsonFile('sync/exports/feature', 'data.diff.json');
        const mappedFeatures = features.deleted?.map((feature: Feature) => this.featureMapper.map(feature));
        ...
        const mappedSpecification = specifications.deleted?.map((specification: Specification) => this.specificationMapper.map(specification, providers.filter((provider: Provider) => provider["Provider Id"] === specification["Provider id"])));

        const processes = readJsonFile('sync/exports/process', 'data.diff.json');
        const mappedProcesses = processes.deleted?.map((process: Process) => this.processMapper.map(process));

        const fillers = readJsonFile('sync/exports/filler', 'data.diff.json');
        const mappedFillers = fillers.deleted?.map((filler: Filler) => this.fillerMapper.map(filler));

        const forms = readJsonFile('sync/exports/form', 'data.diff.json');
        const mappedForms = forms.deleted?.map((form: Form) => this.formMapper.map(form));
        ...
        const mappedQualities = qualities.deleted?.map((quality: Quality) => this.qualityMapper.map(quality));

        const polymerFamily = readJsonFile('sync/exports/polymerfamily', 'data.diff.json');
        const mappedPolymerFamily = polymerFamily.deleted?.map((polymerFamily: PolymerFamily) => this.polymerFamilyMapper.map(polymerFamily));

        const polymerGroups = readJsonFile('sync/exports/polymerGroup', 'data.diff.json');
        const mappedPolymerGroups = polymerGroups.deleted?.map((polymerGroup: PolymerGroup) => this.polymerGroupMapper.map(polymerGroup));

        const polymerSubgroups = readJsonFile('sync/exports/polymersubgroup', 'data.diff.json');
        const mappedPolymerSubgroups = polymerSubgroups.deleted?.map((polymerSubgroup: PolymerSubgroup) => this.polymerSubgroupMapper.map(polymerSubgroup));

        const brands = readJsonFile('sync/exports/brand', 'data.diff.json');
        const brandCountries = readJsonFile('sync/exports/brandcountry', 'data.json');
        const mappedBrands = brands.deleted?.map((brand: Brand) => {
            return this.brandMapper.map(brand, brandCountries.filter((brandCountry: BrandCountry) => brandCountry.brand_id === brand.brand_id));
        });

        const products = readJsonFile('sync/exports/product', 'data.diff.json');
        const mappedProducts = products.deleted?.map((product: Product) => {
        ...
        const caseStudies = readJsonFile('sync/exports/casestudy', 'data.diff.json');

        const filteredCaseStudies = caseStudies.deleted?.filter(cs =>
