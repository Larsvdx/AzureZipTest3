...
export class ContentfulNewAndUpdatedMapperService {
  ...
  mapper(endpoint: string, data: any[], teamDeskData: any[]) {
    let entries: any[] = [];
  ...
  // update product when one of it's feature is updated
  populateProduct(newJson: any[]) {
    if (this.data && this.data.length > 0) {
      const newJsonIds = newJson.flatMap((json: any) => json["pkb_id"]);
      const productInput = this.data.find(
        (data) => data.endpoint === "product",
      );
      const productIndex = this.data.findIndex(
        (data) => data.endpoint === "product",
      );

      const newProductIds = newJsonIds.filter(
        (json: any) =>
          !productInput.diff.newJson
            .map((json: any) => json["pkb_id"])
            .includes(json),
      );
      const newProducts = productInput.teamDeskData.filter((data) =>
        newProductIds.includes(data["pkb_id"]),
      );
      this.data[productIndex].diff.newJson.push(...newProducts);
    }
  }
